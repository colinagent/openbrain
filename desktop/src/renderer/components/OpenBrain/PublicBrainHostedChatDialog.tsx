...
export function PublicBrainHostedChatDialog({ brain, onClose }: { brain: HostedBrain; onClose: () => void }) {
  ...
  const [pending, setPending] = useState<{ question: string; quote: OpenBrainPublicBrainQuote } | null>(null);
  ...
  const prepare = async () => {
    ...
    try {
      ...
      if (quote.funding.kind === 'ai_balance' && quote.maxAuthorizedDebitMicrousd > 0) {
        setPending({ question: text, quote });
        setStatus('');
        setRunning(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-[180] grid place-items-center bg-black/45 p-3" role="dialog" aria-modal="true" aria-labelledby="public-brain-hosted-title">
      <div className="grid h-[min(780px,calc(100vh-24px))] w-[min(820px,calc(100vw-24px))] grid-rows-[auto_1fr_auto] overflow-hidden rounded-2xl border border-border bg-primary-bg shadow-2xl">
          ...
          {messages.length === 0 ? (
            ...
            <article key={message.id} className={`max-w-[84%] rounded-2xl px-4 py-3 ${message.role === 'user' ? 'ml-auto bg-prime-text text-primary-bg' : 'mr-auto border border-border bg-secondary-bg text-prime-text'}`}>
              ...
              {message.role === 'assistant' ? <div className="mt-3 flex flex-wrap gap-x-3 border-t border-border pt-2 font-mono text-[10px] opacity-60">
                ...
                {message.funding?.kind === 'free_daily' ? <span>Free: {message.funding.remaining}/{message.funding.limit} remaining</span> : null}
        ...
        <footer className="border-t border-border bg-secondary-bg/50 p-4">
          {pending ? <div className="mb-3 flex flex-wrap items-center gap-2 rounded-xl border border-[#2f8f6b]/30 bg-[#2f8f6b]/10 p-3 text-xs text-prime-text">
            <span className="min-w-[240px] flex-1">Authorize up to {pending.quote.maxAuthorizedDebitU} U. {mode === 'runtime_byok' ? 'OpenBrain charges only actual Cloud retrieval; your model provider bills generation directly.' : 'Only actual managed-model usage will be charged.'} Access does not include AI usage.</span>
            <button type="button" className="rounded-full bg-prime-text px-3 py-1.5 text-primary-bg" onClick={() => void run(pending.question, pending.quote, conversationID)}>Confirm</button>
            <button type="button" className="rounded-full px-3 py-1.5 text-secondary-text" onClick={() => setPending(null)}>Cancel</button>
          </div> : null}
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
            <div className="inline-flex rounded-full border border-border bg-primary-bg p-1" aria-label="Public brain execution mode">
              <button type="button" disabled={running || Boolean(pending) || messages.length > 0} onClick={() => { setMode('hosted'); setConversationID(''); setConversationMode(null); }} className={`rounded-full px-3 py-1.5 ${mode === 'hosted' ? 'bg-prime-text text-primary-bg' : 'text-secondary-text'}`}>Hosted</button>
              <button type="button" disabled={running || Boolean(pending) || messages.length > 0 || runtimeModels.length === 0} onClick={() => { setMode('runtime_byok'); setConversationID(''); setConversationMode(null); }} className={`rounded-full px-3 py-1.5 ${mode === 'runtime_byok' ? 'bg-prime-text text-primary-bg' : 'text-secondary-text'} disabled:opacity-40`}>Your model</button>
            </div>
            {mode === 'runtime_byok' ? <label className="flex min-w-[220px] flex-1 items-center gap-2 text-secondary-text"><span>Runtime model</span><select value={modelKey} disabled={running || Boolean(pending)} onChange={(event) => setModelKey(event.target.value)} className="min-w-0 flex-1 rounded-lg border border-border bg-primary-bg px-2 py-1.5 text-prime-text"><option value="">Select a configured model</option>{runtimeModels.map((model) => <option key={model.key} value={model.key}>{model.name || model.key}</option>)}</select></label> : null}
          ...
          <form onSubmit={(event) => { event.preventDefault(); void prepare(); }} className="rounded-xl border border-border bg-primary-bg focus-within:border-[#2f8f6b]">
            ...
            <textarea ref={inputRef} id="desktop-public-brain-question" value={question} onChange={(event) => setQuestion(event.target.value.slice(0, 1000))} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows={2} disabled={running || Boolean(pending)} placeholder={`Ask ${brain.name || brain.username}'s public brain…`} className="w-full resize-none bg-transparent px-4 pt-3 text-sm text-prime-text outline-none" autoFocus />
            <div className="flex items-center justify-between px-3 pb-3 pt-1 font-mono text-[10px] text-secondary-text"><span>{question.length}/1000 · {mode === 'runtime_byok' ? 'Your model' : 'Hosted'}</span><button type="submit" disabled={running || Boolean(pending) || !question.trim() || (mode === 'runtime_byok' && !modelKey)} className="rounded-full bg-prime-text px-4 py-2 text-primary-bg disabled:opacity-40">Send</button></div>
          </form>
        </footer>
      </div>
    </div>
...
function publicBrainErrorMessage(error: unknown): string {
  ...
  if (message.includes('free_quota_exhausted')) return 'Today\'s free discovery quota is used. Continue with AI Balance.';
