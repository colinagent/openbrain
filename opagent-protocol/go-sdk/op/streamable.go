...
import (
	...
	"github.com/colinagent/openbrain/opagent-protocol/go-sdk/auth"
	"github.com/colinagent/openbrain/opagent-protocol/go-sdk/internal/jsonrpc2"
	"github.com/colinagent/openbrain/opagent-protocol/go-sdk/jsonrpc"
)
...
func (t *StreamableClientTransport) Connect(ctx context.Context) (Connection, error) {
	...
	conn := &streamableClientConn{
		url:        t.Endpoint,
		client:     client,
		header:     cloneStringMap(t.Header),
		oauth:      t.OAuthHandler,
		incoming:   make(chan jsonrpc.Message, 10),
		done:       make(chan struct{}),
...
// ErrSessionMissing is returned when a Streamable HTTP server reports that the
// current MCP session is no longer present.
//
// The MCP Streamable HTTP transport requires clients to reestablish a session
// after a 404 response for a request carrying an Mcp-Session-Id header.
...
func (c *streamableClientConn) Write(ctx context.Context, msg jsonrpc.Message) error {
	...
	resp, err := c.doHTTPRequest(ctx, req, data)
	if err != nil {
		return fmt.Errorf("%s: %v", requestSummary, err)
	}
...
func (c *streamableClientConn) setConfiguredHeaders(req *http.Request) {
	for key, value := range c.header {
		req.Header.Set(key, value)
	}
}

func (c *streamableClientConn) setAuthorizationHeader(ctx context.Context, req *http.Request) error {
	if c.oauth == nil {
		return nil
	}
...
func (c *streamableClientConn) doHTTPRequest(ctx context.Context, req *http.Request, body []byte) (*http.Response, error) {
	if err := c.setAuthorizationHeader(ctx, req); err != nil {
		return nil, err
	}
	resp, err := c.client.Do(req)
	if err != nil || c.oauth == nil || resp == nil || (resp.StatusCode != http.StatusUnauthorized && resp.StatusCode != http.StatusForbidden) {
		return resp, err
	}
	if err := c.oauth.Authorize(ctx, req, resp); err != nil {
		return nil, err
	}
	retry := req.Clone(ctx)
	if body != nil {
	...
	if err := c.setAuthorizationHeader(ctx, retry); err != nil {
		return nil, err
	}
...
func (c *streamableClientConn) checkResponse(requestSummary string, resp *http.Response) (err error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		// Return ErrSessionMissing to avoid sending a redundant DELETE when the
...
func (c *streamableClientConn) connectSSE(lastEventID string) (*http.Response, error) {
	...
	for ; attempt <= c.maxRetries; attempt++ {
		...
		case <-time.After(calculateReconnectDelay(attempt)):
			...
			resp, err := c.doHTTPRequest(c.ctx, req, nil)
			if err != nil {
	...
	c.closeOnce.Do(func() {
		if errors.Is(c.failure(), ErrSessionMissing) {
			...
			if err != nil {
				...
				c.setConfiguredHeaders(req)
				c.setMCPHeaders(req)
				if _, err := c.doHTTPRequest(c.ctx, req, nil); err != nil {
					c.closeErr = err
				}
			}
		}
