...
type testOAuthHandler struct {
	...
	err         error
	calls       int
	check       func(*http.Request, *http.Response)
}
...
func (h *testOAuthHandler) Authorize(_ context.Context, req *http.Request, res *http.Response) error {
	h.calls++
	if h.check != nil {
		h.check(req, res)
	}
...
func TestHTTPTransport(t *testing.T) {
	...
	t.Run("successful auth flow", func(t *testing.T) {
		handler := &testOAuthHandler{
			...
			check: func(_ *http.Request, res *http.Response) {
		...
		if handler.calls != 1 {
			t.Errorf("handler was called %d times, want 1", handler.calls)
		}
		...
		if handler.calls != 1 {
			t.Errorf("handler should still be called only once, but was %d", handler.calls)
		}
	})
	...
	t.Run("handler returns error", func(t *testing.T) {
		...
		handler := &testOAuthHandler{err: handlerErr}
