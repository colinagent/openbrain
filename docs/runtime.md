...
Desktop never asks a user to type an organization id or slug. A normal sign-in
finishes against the user's personal organization, then reads the authenticated
organization directory. One available organization continues automatically;
multiple organizations require an explicit choice. The account menu always
shows the current organization and exposes the same choices later. Selecting an
enterprise that requires stronger SSO starts an organization-targeted sign-in
with the server-provided slug rather than asking the user to copy it.

User-facing organization labels never expose the opaque organization id or
slug. A personal organization is presented as **Personal workspace**; an
enterprise is presented by its configured name and the member's role. The
selector keeps its organization list scrollable within short Desktop windows so
every valid membership remains reachable.
