...
The proprietary OpenBrain host connector consumes this public contract and
connects outward to the selected relay region. Its implementation is not part
of this repository. The relay contract does not expose arbitrary local HTTP
routes or runtime opcodes: requests pass through a default-deny dispatcher and
...
Host-implementation conformance tests live with the proprietary Desktop server.
